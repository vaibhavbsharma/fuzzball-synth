...
my $bin = "./WhackJack";
die "Usage: synth-argsub-repair.pl <arch=x86 only> <seed> <default adapter(0=zero,1=identity) <ret-adapter=1, any other value otherwise>[<lower bound for constant> <upper bound for constant>] <verbose=1, non-verbose=0, extra-verbose=2 (is logging-heavy, be warned)>"
  unless @ARGV == 7;
my($arch, $rand_seed, $default_adapter_pref, $ret_adapter_on, $const_lb, $const_ub, $verbose) = @ARGV;
...
my @crash_cond_opt = ("-disqualify-path-on-nonfalse-cond",
		      # next 6 conditions check if cgc_atoi is reading a character from buffer outside its allocated length
		      "-check-condition-at", "0x0804b4ad:R_EAX:reg32_t\>=0xbfffcdc8:reg32_t\&R_ESP:reg32_t==0xbfffcd50:reg32_t", # 1 
		      "-check-condition-at", "0x0804b4d2:R_EAX:reg32_t\>=0xbfffcdc8:reg32_t\&R_ESP:reg32_t==0xbfffcd50:reg32_t", # 2
		      "-check-condition-at", "0x0804b4ed:R_EAX:reg32_t\>=0xbfffcdc8:reg32_t\&R_ESP:reg32_t==0xbfffcd50:reg32_t", # 3
		      "-check-condition-at", "0x0804b511:R_EAX:reg32_t\>=0xbfffcdc8:reg32_t\&R_ESP:reg32_t==0xbfffcd50:reg32_t", # 4
		      "-check-condition-at", "0x0804b52c:R_ECX:reg32_t\>=0xbfffcdc8:reg32_t\&R_ESP:reg32_t==0xbfffcd50:reg32_t", # 5
		      "-check-condition-at", "0x0804b52c:R_ECX:reg32_t\>=0xbfffcdc8:reg32_t\&R_ESP:reg32_t==0xbfffcd50:reg32_t", # 6
		      "-check-condition-at", "0x08048bde:R_EDI:reg32_t\>=0x3:reg32_t", # checks offset within buffer in cgc_receive_until 
		      "-check-condition-at", "0x08048c67:R_EAX:reg32_t\>=0x4:reg32_t", # checks offset within buffer in cgc_receive_until 
		      "-tracepoint",  "0x0804d44f:R_EAX:reg32_t", # prints number of characters read by cgc_receive_until 
		      "-tracepoint", "0x08048c36:R_EAX:reg32_t"); # prints value read from read system call

my $receive_until_delim_jne_eip = "0x08048c3c";
my @branch_pref_opts = ("-branch-preference","$receive_until_delim_jne_eip:0");
...
my @repair_opts = ( # TODO automate the inference of this target fragment selection
    "-repair-frag-start", "0x0804d41b",  
    "-repair-frag-end", "0x0804d46c");
...
my @common_opts = (
    ...
    "-f1-iteration-limit", 5,
    "-f2-iteration-limit", 5,
    @repair_opts);
...
sub try_synth {
    ...
    my @args = ($fuzzball, "-linux-syscalls", "-arch", $arch, $bin,
		...
		# "-extra-condition", "b_is_const:reg1_t==0x0:reg1_t", "-extra-condition", "b_val:reg32_t==0x00000001:reg32_t",,
		# "-extra-condition", "c_is_const:reg1_t==0x1:reg1_t", "-extra-condition", "c_val:reg32_t==0x00000004:reg32_t",,
		# "-extra-condition", "d_is_const:reg1_t==0x0:reg1_t", "-extra-condition", "d_val:reg32_t==0x00000003:reg32_t",,
		# "-extra-condition", "repair_EIP:reg64_t==0x0804d44a:reg64_t",
