...
long double const_lb = -256;
long double const_ub = 256;
long double f1nargs = 4;
int f2nargs = 4;
...
long double num_argsub() {
  ...
  //printf("\n");
  // no return val. sub. adapter is currently used in repair
  long double R = 1; //8; //8*f2nargs + 9 + D + 1;
  return ret * R;
}
