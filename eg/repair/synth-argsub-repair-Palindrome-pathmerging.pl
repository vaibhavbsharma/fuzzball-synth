...
my $input_len = 64;
my $bin = "./Palindrome-pm$input_len";
...
my $strlen_check_reg = "R_EDX";
my $cgc_receive_delim_overflow_eip = "0x08048938";
my $cgc_receive_delim_overflow_reg = "R_EBX";
my $strlen_check_overflow_cond = sprintf("%s:reg32_t\>=\$0x%x:reg32_t", $strlen_check_reg, $input_len);
my $cgc_receive_delim_overflow_cond = sprintf("%s:reg32_t\>=\$0x%x:reg32_t", $cgc_receive_delim_overflow_reg, $input_len);
my @crash_cond_opt = ("-disqualify-path-on-nonfalse-cond", 
		      "-check-condition-at", $strlen_check_eip . ":" . $strlen_check_overflow_cond, # checks OOB access when calculating string length in cgc_check
		      "-check-condition-at", $cgc_receive_delim_overflow_eip . ":" . $cgc_receive_delim_overflow_cond, # checks OOB access when writing to passed argument string in cgc_receive_delim 
		      "-tracepoint", $strlen_check_eip . ":" . "$strlen_check_reg:reg32_t", 
		      "-tracepoint", sprintf("%s:R_EDX:reg32_t", $input_len < 64 ? "0x080489d0" : "0x080489d3"), # checks value of length
		      "-tracepoint", "0x0804895c:mem[R_ESI:reg32_t]:reg8_t"); # prints value read from read system call

my $strlen_check_jne_eip = $input_len < 64 ? "0x080489ce" : "0x080489d1";
my $delim_check_je_eip = "0x0804895e";
...
sub try_synth {
    ...
    my @args = ($fuzzball, "-linux-syscalls", "-arch", $arch, $bin,
		...
		# "-extra-condition", "a_is_const:reg1_t==0x0:reg1_t", "-extra-condition", "a_val:reg32_t==0x0000:reg32_t",
		# "-extra-condition", "b_is_const:reg1_t==0x0:reg1_t", "-extra-condition", "b_val:reg32_t==0x0001:reg32_t",
		# "-extra-condition", "c_is_const:reg1_t==0x1:reg1_t", "-extra-condition", "c_val:reg32_t==0x0040:reg32_t",
		# "-extra-condition", "d_is_const:reg1_t==0x0:reg1_t", "-extra-condition", "d_val:reg32_t==0x0003:reg32_t",
		# "-extra-condition", "repair_EIP:reg64_t==$cgc_receive_delim_call_addr:reg64_t",
