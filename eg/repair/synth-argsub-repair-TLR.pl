...
my $check_win_overflow_reg = "R_EAX"; 
my $receive_until_overflow_eip = "0x08049f69"; # value of local variable len loaded by cgc_receive_until into EDI
my $receive_until_overflow_reg = "R_EDI";
my $check_win_overflow_cond = sprintf("%s:%s:reg32_t\>=\$0x%x:reg32_t", $check_win_overflow_eip, $check_win_overflow_reg, $input_len);
my $receive_until_overflow_cond = sprintf("%s:%s:reg32_t\>=\$0x%x:reg32_t", $receive_until_overflow_eip, $receive_until_overflow_reg, ($input_len-1));
my @crash_cond_opt = ("-disqualify-path-on-nonfalse-cond", 
		      "-check-condition-at", $check_win_overflow_cond, # checks OOB access when writing 0 to end of string
		      "-check-condition-at", $receive_until_overflow_cond, # checks OOB access when receiving string over read syscall 
		      "-tracepoint", $check_win_overflow_eip . ":" . "$check_win_overflow_reg:reg32_t", 
		      "-tracepoint", $receive_until_overflow_eip . ":" . "$receive_until_overflow_reg:reg32_t", 
		      "-tracepoint", "0x08049fc5:R_EAX:reg32_t"); # prints value read from read system call

my $receive_until_delim_jne_eip = "0x08049fb1";
...
my $cgc_check_call_addr =
    "0x" . substr(`objdump -dr $bin | grep 'call.*<cgc_check_win>'`, $obj_start, $obj_end);
...
sub try_synth {
    ...
    my @args = ($fuzzball, "-linux-syscalls", "-arch", $arch, $bin,
		...
		# "-extra-condition", "repair_EIP:reg64_t==0x0804cae2:reg64_t",
