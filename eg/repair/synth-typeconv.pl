...
my $bin = "./two-funcs";
die "Usage: synth-one.pl <arch=x86 or x64> <f1num> <f2num> <seed> <default adaptor(0=zero,1=identity) [<lower bound for constant> <upper bound for constant>] <recompile $bin=1, use $bin as-is=0> <verbose=1, non-verbose=0, extra-verbose=2 (is logging-heavy, be warned)>"
  unless @ARGV == 9;
my($arch, $f1num, $f2num, $rand_seed, $default_adaptor_pref, $const_lb, $const_ub, $recompile, $verbose) = @ARGV;
...
if ($recompile == 1) {
    ...
    my $unused = $arch eq "x86" ? `gcc -m32 -static two-funcs.c -g -o two-funcs` : `gcc -static two-funcs.c -g -o two-funcs`;
    my $gcc_ec = $?;
...
while (<F>) {
    my($nargs, $name, $type) = split(" ", $_, 3);
    if ($nargs =~ /\+/) {
	$nargs = substr($nargs,0,1);
    }
    push @func_info, [$nargs, $name, $type];
}
close F;
...
my $fuzz_start_addr = "0x" . substr(`nm $bin | fgrep " T fuzz_start"`, 0, $arch eq "x64" ? 16 : 8);

my $f1_addr = "0x" . substr(`nm $bin | fgrep " T f1"`, 0, $arch eq "x64" ? 16 : 8);

my ($obj_start,$obj_end) = $arch eq "x64" ? (2,6) : (1,7);

my $f1_call_addr =
  "0x" . substr(`objdump -dr $bin | grep 'call.*<f1>'`, $obj_start, $obj_end);

my $f2_addr = "0x" . substr(`nm $bin | fgrep " T f2"`, 0, $arch eq "x64" ? 16 : 8);

my $wrap_f2_addr = "0x" . substr(`nm $bin | fgrep " T wrap_f2"`, 0, $arch eq "x64" ? 16 : 8);

my $f2_call_addr =
  "0x" . substr(`objdump -dr $bin | grep 'call.*<wrap_f2>'`, $obj_start, $obj_end);
...
for my $i (0 .. 5) {
    $arg_addr[$i] =
      "0x" . substr(`nm $bin | fgrep " B global_arg$i"`, 0, $arch eq "x64" ? 16 : 8);
}
my $symbolic_arg_opt = $arch eq "x64" ? "-symbolic-long" : "-symbolic-word";

my $match_jne_addr =
  "0x" . substr(`objdump -dr $bin | grep 'jne.*compare+'`, $obj_start, $obj_end);
...
printf "%d = %s(%d)\n", $f1num, $func_info[$f1num][1], $func_info[$f1num][0];
printf "%d = %s(%d)\n", $f2num, $func_info[$f2num][1], $func_info[$f2num][0];
...
if($const_lb != $const_ub) {
	...
	my $s2='';
	if($f1nargs != 0) {
	    $s1 = sprintf("%s_type:reg8_t==0:reg8_t | %s_val:%s>=\$0x%Lx:%s",$n,$n,$reg_size,$const_lb,$reg_size);
	    $s2 = sprintf("%s_type:reg8_t==0:reg8_t | %s_val:%s<=\$0x%Lx:%s",$n,$n,$reg_size,$const_ub,$reg_size);
	}
	else {
...
sub check_adaptor {
    ...
    my @args = ($fuzzball, "-linux-syscalls", "-arch", $arch,
		...
		$symbolic_arg_opt, "$arg_addr[0]=a",
		$symbolic_arg_opt, "$arg_addr[1]=b",
		$symbolic_arg_opt, "$arg_addr[2]=c",
		$symbolic_arg_opt, "$arg_addr[3]=d",
		$symbolic_arg_opt, "$arg_addr[4]=e",
		$symbolic_arg_opt, "$arg_addr[5]=f",
