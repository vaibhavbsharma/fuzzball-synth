...
sub report_metrics {
    ...
    my $total_runtime = 600;

    print "Starting analysis for $fn_name\n";

    for(my $i=1; $i <= $num_dirs; $i++) {
	open(LOG,"<$dir_path/glibc-$i/logs/$log_file_name");
	my ($result,$count) = (-1,0);
	my $saved_cmd = "";
	my ($line_num) = (0);
	my @saved_result_str = ();
	while(<LOG>) {
		...
		if(scalar(@timeout_time)/5 != scalar(@timeout_solver)/5 ||
		   ...
		   scalar(@adaptor_solver)/5 != scalar(@adaptor_steps)/3) {
		    die sprintf("file($dir_path/glibc-$i/logs/$log_file_name), line($line_num), number of timeouts or inequiv or adaptor instances dont match between time, solver, or steps" . 
...
report_metrics($dir_path, $log_file_name, $num_dirs);
print "$timeout_table\n";
print "$inequiv_table\n";
print "$adaptor_table\n";
