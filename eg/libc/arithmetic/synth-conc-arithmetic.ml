...
let conc_adaptor_bin =
  ignore(syscall "gcc -static -g -o two-funcs-conc two-funcs-conc.c -lpthread");
  match syscall ("pwd") with
  | [str] -> str^"/two-funcs-conc"
  | _ -> failwith "could not run pwd"

let side_effects_equal_addr = 
  match syscall ("nm "^conc_adaptor_bin^" | fgrep ' B sideEffectsEqual'") with
  | [str] -> "0x" ^ String.sub str 0 16
  | _ -> failwith "Unexpected sideEffectsEqual address format"
...
let try_synth () = 
  ...
  let cmd = 
    ["pin -t obj-intel64/PinMonitor.so -- "^conc_adaptor_bin; 
     (string_of_int f1num); (string_of_int f2num); 
     "f tests"; (string_of_int const_lb); (string_of_int const_ub); 
     side_effects_equal_addr; "3 1"]
     in
  let _ = printf "%s\n%!" (String.concat " " cmd) in
  let ic = Unix.open_process_in (String.concat " " cmd) in
...
let find_all_adaptors () =
  let cmd = 
    ["pin -t obj-intel64/PinMonitor.so -- "^conc_adaptor_bin; 
     (string_of_int f1num); (string_of_int f2num); 
     "f tests"; (string_of_int const_lb); (string_of_int const_ub); 
     side_effects_equal_addr; "3 0"]
     in
  let _ = printf "%s\n%!" (String.concat " " cmd) in
  let ic = Unix.open_process_in (String.concat " " cmd) in
  (* read_results : string list -> bool -> string
...
let rec main () =
  ...
  | (true, _) -> (* we found a suitable adaptor *)
      ...
      find_all_adaptors ();
      printf "find-all-adaptors took %fs seconds\n" (Unix.gettimeofday () -. end_ver);
      printf "Success!\nFinal test set:\n%!";
      print_tests ();
