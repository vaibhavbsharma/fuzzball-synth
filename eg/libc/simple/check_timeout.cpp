...
int main(int argc, char * argv[]) {
  ...
  for(int i=0;i<2;i++) {
	...
	if(line.find(blacklisted_fns[ign_ind])!=string::npos) {
      ...
      if(line.find("Mismatch")!=string::npos) {
	ce_found = 1;
      }
      if(line.find("alarm")!=string::npos && line.length() == 5) {
	total_fp_count++;
	if(ignore_flag==0) {
	  ...
	  if(ce_found==1) {
	    cout<<f1_name<<"("<<f1<<") "<<f2_name<<"("<<f2<<") ce found"<<endl;
	    ce_found_count++;
	  }
	  else 
	    cout<<f1_name<<"("<<f1<<") "<<f2_name<<"("<<f2<<") no ce found"<<endl;
	}
  ...
  cout<<"ce found count = "<<ce_found_count<<endl;
