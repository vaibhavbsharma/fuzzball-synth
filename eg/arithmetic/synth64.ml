...
let print_adaptor () =
  ...
  List.iter (fun el -> let l = Str.split (Str.regexp " \\|:\\|=") el in
                       ...
                       printf "%s = %s%!\n" name valu (*(slightly_better_print name valu)*))
            !adapt
  ...
  then
    let rec loop l n =
      ...
      | (h::t) -> let x = String.make 1 (Char.chr ((Char.code 'a') + n)) in
                  ...
                  :: (loop t (n+1))
    in loop (syscall ("nm " ^ bin ^ " | fgrep \" B global_\" ")) 0
  else
...
let check_adaptor () = 
  ...
  let rec read_results (matches, fails) record_ce f1_completed bad_adaptor = 
    try
      ...
      match line with
      | "Match" -> read_results (matches + 1, fails) record_ce f1_completed bad_adaptor
      | "Mismatch" -> read_results (matches, fails + 1) true f1_completed bad_adaptor
      | "Completed f1" -> read_results (matches, fails) record_ce true bad_adaptor
      | _ when (match_regex line ".*expression violates restriction.*") ->
            read_results (matches, fails) record_ce f1_completed true
      | _ when (match_regex line "Stopping at null deref at 0x[0-9a-f]+") 
                  && f1_completed ->
            read_results (matches, fails + 1) true f1_completed bad_adaptor
      ...
      | _ when (match_regex line "Stopping on disqualified path at 0x[0-9a-f]+") 
                  && f1_completed && (not bad_adaptor) ->
            read_results (matches, fails + 1) true f1_completed bad_adaptor
      | _ when (match_regex line "Stopping on disqualified path at 0x[0-9a-f]+") 
                  && bad_adaptor ->
            read_results (matches, fails) record_ce f1_completed false
          ...
          List.iter 
            (fun v ->
               ...
               then let idx = (Char.code (String.get v 0)) - (Char.code 'a') in
                    let value = 
                      Int64.of_string (String.sub v 2 ((String.length v) - 2)) in
      ...
      | _ -> read_results (matches, fails) record_ce f1_completed bad_adaptor
    with End_of_file -> 
      let _ = Unix.close_process_in ic in
      if matches = 0 && fails = 0 
      then failwith "Missing results from check run"
      else (true, Array.to_list !ce) in
  read_results (0, 0) false false false
          ...
          List.iter 
            (fun v ->
               ...
               then match Str.split (Str.regexp "=") v with
                    ...
                    | name::value::[] -> 
                        specified_vals := 
                          (name, Int64.of_string value) :: !specified_vals
