...
die "Usage: synth.pl <bucket number ( 1 - 8 ) <direction ( 1 or -1 )>" unless @ARGV == 2;
my($bucket_num, $direction) = @ARGV;

my $total_fns = 1316;
my @boundaries;
for my $i (0 .. 8) {
    push @boundaries, int($i * $total_fns/8);
}
$boundaries[0]=-1;

my $start_lim = $boundaries[$bucket_num-1]+1;
my $end_lim = $boundaries[$bucket_num];
printf "start_lim = $start_lim end_lim = $end_lim direction = $direction number of functions = %d\n", $end_lim-$start_lim;
