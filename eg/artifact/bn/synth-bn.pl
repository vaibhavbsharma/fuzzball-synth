...
sub reinitialize_matrix_addrs {
    ...
    push @a_addrs,
    sprintf("0x%x=0x%x", $a_addr, 0);
    push @a_addrs, "-store-word";
    push @a_addrs,
    sprintf("0x%x=0x%x", $a_addr+8, 0);
    push @a_addrs, "-store-word";
    push @a_addrs,
    sprintf("0x%x=0x%x", $a_addr+12, 0);
    push @a_addrs, "-store-word";
    push @a_addrs,
    sprintf("0x%x=0x%x", $a_addr+16, 0);
    push @a_addrs, "-store-word";
    push @a_addrs,
    sprintf("0x%x=0x%x", $a_addr+20, 1);
}
...
sub check_adaptor {
    ...
    # push @tmp_str, sprintf("a:reg64_t == 0x%x:reg64_t", $a_addr);

    my @tmp_str;
    push @tmp_str, "-check-condition-at";
    #push @tmp_str, "0x4099ea:mem[mem[R_RBP:reg64_t-0x10:reg64_t]:reg64_t+0xc:reg64_t]:reg32_t]";
    push @tmp_str, "0x4098ff:mem[R_RCX:reg64_t+0xc:reg64_t]:reg32_t>mem[R_RBP:reg64_t-0x2c:reg64_t]:reg32_t";
    # push @tmp_str, "-tracepoint";
    # push @tmp_str, "0x4099ea:mem[R_RBP:reg64_t-0x2c:reg64_t]:reg32_t"; 
