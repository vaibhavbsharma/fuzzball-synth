...
die "Usage: synth-one <random-seed>" if @ARGV != 1;
my($rand_seed) = @ARGV;
...
my @synth_ret_opt = ("-synthesize-return-adaptor",
		     join(":", "return-simple+len",
			  $f2_call_addr, $post_f2_call, $f2nargs));


my @common_opts = ("-table-limit", 8, "-implied-value-conc");

my($const_lb, $const_ub) = (0, 1);
my @const_bounds_ec = ();
...
sub check_adaptor {
    ...
    my @args = ($fuzzball, "-linux-syscalls", "-arch", "x64",
		...
		@synth_opt, @synth_ret_opt, @conc_adapt, @const_bounds_ec,
	...
	if ($_ eq "Match\n" ) {
	    ...
	    $f1_completed = 1;
	    $skip_print = 1;
	} elsif (/^Stopping when program called exit/) {
	    $skip_print = 1;
	} elsif ($_ eq "Starting simple adaptor\n") {
	    $skip_print = 1;
	} elsif (($_ eq "Mismatch\n") or 
...
sub try_synth {
    ...
    my @args = ($fuzzball, "-linux-syscalls", "-arch", "x64", $bin,
		...
		"-trace-iterations", "-trace-assigns", "-solve-final-pc",
		"-return-zero-missing-x64-syscalls",
		@synth_opt, @synth_ret_opt, @const_bounds_ec,
		"-match-syscalls-in-addr-range",
		$f1_call_addr.":".$post_f1_call.":".$f2_call_addr.":".$post_f2_call,
