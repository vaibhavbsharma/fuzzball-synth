...
int32_t ref9(int32_t x, int32_t y)
{
    int64_t temp;
    temp = x;
    temp *= y;
...
unsigned char ref11(unsigned char c)
{
   if (c >= 'A' && c <= 'F')
      return (c - 'A') + 10;
...
int ref12( unsigned angle, bool hflip, 
...
size_t ref14(size_t x, size_t y, size_t w, size_t h)
{
    size_t flim = x + (y & ~1) * w;
    if (y & 1) {
        flim += (x & ~3) + 2;
...
bool ref15( uint32_t u_n, uint32_t u_m)
{
...
unsigned int ref19(unsigned int);
unsigned int ref17(unsigned int x)
{
  x |= (x >> 1);
  x |= (x >> 2);
  x |= (x >> 4);
...
unsigned int ref18(unsigned int v) {
  unsigned int c = 32;
  v &= - ((int) (v));
  if (v) c--;
...
unsigned int ref19(unsigned int x) {
  x -= ((x >> 1) & 0x55555555);
...
unsigned int ref20(unsigned int v) {
  v ^= v >> 16;
  v ^= v >> 8;
  v ^= v >> 4;
  v &= 0xf;
...
uint16_t ref21( uint16_t sample )
{
...
//small function in /export/scratch/vaibhav/vlc-2.2.6/modules/demux/mpeg/es.c: 
//int MpgaGetFrameSamples( uint32_t h )
int ref25( uint32_t h )
{
...
/***** start of uninteresting reference functions ****/
...
// causes problems with the modulus operation
int MOD(int a, int b) {
...
// always times out because of the solver
