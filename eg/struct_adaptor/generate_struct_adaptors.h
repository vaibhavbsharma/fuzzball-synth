...
void generate_struct_adaptors_randomized(long start, int fieldnum) {
  ...
  if(fieldnum == number_of_fields) {
    ...
    //f2's field layout could be a narrowing permutation of f1's field layout
  ...
  int i, j, k, end;
  for(end=start; end<=region_limit-(number_of_fields-1-fieldnum); end++) {
    int n, sz; 
    long start1;
